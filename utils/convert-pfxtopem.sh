...
function Convert-PFXtoPEM () {
    ...
    # Convert/Extract the PFX file to DER Formatted Certificate file
    openssl pkcs12 -in "${FILE}" -clcerts -nokeys -passin pass:"${PFXPass}" | openssl x509 -outform der -out "${FILE::-4}.der"
